...
# Tanh function
def tanh(x):
    t = [(math.exp(x[kk]) - math.exp(-x[kk])) / (math.exp(x[kk]) + math.exp(-x[kk])) for kk in range(len(x))]
    return t
...
def softmax(x):
    max_x = max(x[kk])
    exp_x = [math.exp(i - max_x) for i in x[kk]]
    sum_exp_x = sum(exp_x)
...
yout2 = dense(3, yout1, w2, b2, 'tanh') # hidden layer with 3 neurons
yout3 = dense(2, yout2, w3, b3, 'softmax') # hidden layer with 2 neurons
